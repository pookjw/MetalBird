...
void ObstacleRenderer::drawInRenderEncoder(id<MTLRenderCommandEncoder> renderEncoder, CGSize size) {
    ...
    const std::int16_t obstaclesCount = static_cast<std::int16_t>(std::fmaf(size.width, std::powf(std::fmaf(ObstacleRenderer::obstaclesAbsoluteSpacing, 1.f, ObstacleRenderer::obstaclesAbsoluteSpacing), -1.f), 0.f));
    
    std::vector<simd_float2> obstacles (obstaclesCount * 8);
    std::vector<std::uint16_t> indices (obstaclesCount * 12);
    
    const std::float_t obstaclesRelativeSpacing = std::fmaf(ObstacleRenderer::obstaclesAbsoluteSpacing, std::powf(size.width, -1.f), 0.f);
    const std::float_t obstacleRelativeWidth = std::fmaf(ObstacleRenderer::obstacleAbsoluteWidth, std::powf(size.width, -1.f), 0.f);
